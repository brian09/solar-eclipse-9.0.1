...
Eigen_Data::Eigen_Data(vector<string> _ids, const char * base_eigen_data_filename, const size_t _n_sets){
	...
	throw Eigen_Data_Exception(cerror_message);
    }   
    ...
    n_subjects = ids.size();
    if(n_subjects == 0){
        throw Eigen_Data_Exception("Subject ID count is zero");
    } 
    ...
    for(int row = 0 ; row < n_subjects; row++){
        double eigenvalue;
        if(eigenvalues_stream.eof()){
            throw Eigen_Data_Exception("Missing eigenvalue in eigen data");
        }
        eigenvalues_stream >> eigenvalue;
        eigenvalues[row] = eigenvalue;
        for(int col = 0 ; col < n_subjects; col++){
            if(eigenvectors_stream.eof()){
              
                throw Eigen_Data_Exception("Missing eigenvector value in eigen data");
            }
                double eigenvector_value;
                eigenvectors_stream >> eigenvector_value;
                eigenvectors_transposed[col*n_subjects + row] = eigenvector_value;                          
        }
    }
...
Eigen_Data::Eigen_Data(vector<string> all_ids, vector<string> skip_ids, const size_t _n_sets){
    ...
    if(errmsg){
        throw Eigen_Data_Exception(errmsg);
    }
    ...
    if(ids.size() == 0){
        errmsg = "Pedigree and Phenotype IDs do not match";
        throw Eigen_Data_Exception(errmsg);
    }
    ...
    if(static_phi2 == 0){
        throw Eigen_Data_Exception("Load phi2 matrix was not called prior to running Eigen_Data constructor");
    }
    n_subjects = ids.size();
    phenotype_buffer = new double[n_subjects*n_phenotypes];
    trait_names.resize(n_phenotypes);
...
Solar_Trait_Reader::Solar_Trait_Reader(const char * phenotype_filename, const char * base_eigen_data_filename, vector<string> trait_names){
    ...
    if(eigen_data_ids.size() == 0){
        throw Solar_Trait_Reader_Exception("No subject IDs were found in .ids file for eigen data file set");
    }
    ...
    if(n_phenotypes < OPENMP_THRESHOLD){
        const char * errmsg = 0;
        ...
        string thread_error_messages[MAX_OMP_THREADS];
...
#pragma omp parallel
        {
            ...
            SolarFile * file = SolarFile::open("Multi-Trait-Read", phenotype_filename, &errmsg);
            if(errmsg){
                thread_error_messages[thread_index] = errmsg;
#pragma omp critical
                {
                    error_count++;
                }
                ...
                try{
                    ...
                    thread_error_messages[thread_index] = e.what();
    #pragma omp critical
                    {
                        error_count++;
                    }
    ...
    #pragma omp critical
        ...
        if(error_count != 0){
            string str_error_message = "Multithreading errors during phenotype read:\n";
            for(int i = 0 ; i < MAX_OMP_THREADS; i++){
                if(thread_error_messages[i].length() != 0){
                    str_error_message += "Thread " + to_string(i) + ": " + thread_error_messages[i] + "\n";
                }
            }
            throw Solar_Trait_Reader_Exception(str_error_message.c_str());
        }        
    }
    ...
    if(excluded_trait_names.size() != trait_names.size()){ 
      ...
      try{
   	 ...
   	 }catch(Eigen_Data_Exception & e){
           string str_error_message = "Error reading eigen data: " + string(e.what());
           throw Solar_Trait_Reader_Exception(str_error_message.c_str());         
     }
	 ...
	 n_phenotypes = trait_names.size() - excluded_trait_names.size();
	   ...
	   n_sets = 0;
	   n_phenotypes = 0;
       const char *error_message = "No of the traits selected had an ID set that matched that of the EVD data ID set";
       throw Solar_Trait_Reader_Exception(error_message);
	   eigen_data = 0;
     }
     delete [] phenotype_buffer;
}
...
#pragma omp critical
    ...
    if(error_count != 0){
        ...
        throw Solar_Trait_Reader_Exception(str_error_message);
    }
    }
...
//#pragma omp parallel for
    for(size_t data_index = 0; data_index < n_sets; data_index++){
        try{
           ...
           string str_error_message = "Error reading eigen data for data index " + to_string(data_index) + ": " + e.what();
           throw Solar_Trait_Reader_Exception(str_error_message.c_str()); 
        }
    }
